...
public:
    ...
    void loadFile(std::string fileRaw, std::map<int, Patient>& patien,
        std::map<int, Patient>& dia,
        std::map<int, Patient>& hyperte,
        std::map<int, Patient>& alcho,
        std::map<int, Patient>& schola,
        std::map<int, Patient>& handic, int& num4ID) {
        ...
        crow::json::rvalue jsonLoad = crow::json::load(fileRaw);
        ...
        patien = patients;
        dia = diabetics;
        hyperte = hypertensions;
        alcho = alcholics;
        schola = scholars;
        handic = handicaps;
        num4ID = patients.size();
    }
};
...
int main()
{
    ...
    int numId = 0;
    ob.loadFile(s, patients, diabetics, hypertensions, alcholics, scholars, handicaps, numId);
    //params
    ...
    CROW_ROUTE(app, "/addPatient")
        ([&diabetics, &alcholics, &handicaps, &hypertensions, &scholars, &patients, &params2, &numId](const crow::request& req) {
        crow::json::wvalue x;
        auto updates = req.raw_url;
    ...
    CROW_ROUTE(app, "/logout")
        .methods("GET"_method)
        ([&patients, &fileNum](const crow::request& req) {
        crow::response res;
        res.set_header("Access-Control-Allow-Origin", "*");
        res.set_header("Access-Control-Allow-Methods", "GET");
        res.set_header("Access-Control-Allow-Headers", "Content-Type");
        ...
        std::string stuff = "/Users/mukul/Desktop/Drive/backend/test" + std::to_string(fileNum) + ".json";
        std::ofstream outputFile(stuff);
        fileNum++;
    ...
    CROW_ROUTE(app, "/import")
        ...
        ([&importString, &ob, &patients, &diabetics, &alcholics, &handicaps, &hypertensions, &scholars, &numId](const crow::request& req) {
        crow::response res;
        std::string filedata;
        try {
            ...
            res.code = 500;
            res.body = "An error occurred while processing the file.";
        }
        size_t startPos = filedata.find('[');
        size_t endPos = filedata.find(']');

        if (startPos != std::string::npos && endPos != std::string::npos && endPos > startPos) {
            importString = filedata.substr(startPos, endPos - startPos+1);
        }
        else {
            importString = "";  // If start and end markers not found, set importString to empty
        }

        ob.loadFile(importString, patients, diabetics, hypertensions, alcholics, scholars, handicaps, numId);
        return res;
            });
...
// /Users/mukul/Desktop/Drive/backend/test.json
